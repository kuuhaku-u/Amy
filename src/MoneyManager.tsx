...
export default function MoneyManager({ date, sheet, token, online }: { date: string; sheet: string; token: string; online: boolean }) {
  ...
  useEffect(() => { try { const saved = JSON.parse(localStorage.getItem(key) || "null") || initial; setSettings({ ...saved, income: 0 }); setIncomeDraft(""); setIncomeUnlocked(false); } catch { setSettings(initial); } }, [key]);
  ...
  const unlockIncome = async () => { setIncomeStatus("Unlocking with your app access key…"); try { const result = await unlockPrivateIncome(date.slice(0, 7), token); setSettings((current) => ({ ...current, income: result.income })); setIncomeDraft(String(result.income)); setIncomeUnlocked(true); setIncomeStatus("Income unlocked for this session."); } catch (error) { setIncomeStatus(error instanceof Error ? error.message : "Could not unlock income."); } };
  const storeIncome = async () => { if (!Number(incomeDraft)) { setIncomeStatus("Enter a monthly income."); return; } setIncomeStatus("Encrypting with your app access key…"); try { const result = await savePrivateIncome(date.slice(0, 7), Number(incomeDraft), token); setSettings((current) => ({ ...current, income: result.income })); setIncomeUnlocked(true); setIncomeStatus("Encrypted income saved to Google Sheets."); } catch (error) { setIncomeStatus(error instanceof Error ? error.message : "Could not save income."); } };
  const lockIncome = () => { setSettings((current) => ({ ...current, income: 0 })); setIncomeDraft(""); setIncomeUnlocked(false); setIncomeStatus("Income locked."); };
  ...
  return <section className="manager-view">
    ...
    {section === "dashboard" && <>
      <article className="income-entry-card"><div><span className="eyebrow">Private monthly income</span><h3>{incomeUnlocked ? "Income unlocked" : "Income is hidden"}</h3><p>Your one-time app login key also protects the encrypted income.</p></div><div className="private-income-controls"><div className="income-entry-control"><span>₹</span><input inputMode="decimal" placeholder={incomeUnlocked ? "Monthly income" : "Enter income to set it"} value={incomeDraft} onChange={(e) => { if (/^\d*(\.\d{0,2})?$/.test(e.target.value)) setIncomeDraft(e.target.value); }} /></div><div>{!incomeUnlocked && <button onClick={unlockIncome}>Unlock with login key</button>}<button onClick={storeIncome}>{incomeUnlocked ? "Update securely" : "Set new income"}</button>{incomeUnlocked && <button className="lock-income" onClick={lockIncome}>Lock</button>}</div>{incomeStatus && <small>{incomeStatus}</small>}</div></article>
