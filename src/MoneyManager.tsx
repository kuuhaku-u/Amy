...
import { loadAnalytics, loadCashflow, loadHistory, savePrivateIncome, unlockPrivateIncome } from "./api";
...
export default function MoneyManager({ date, sheet, token, online }: { date: string; sheet: string; token: string; online: boolean }) {
  ...
  const [incomePassphrase, setIncomePassphrase] = useState("");
  const [incomeUnlocked, setIncomeUnlocked] = useState(false);
  const [incomeStatus, setIncomeStatus] = useState("");

  useEffect(() => { try { const saved = JSON.parse(localStorage.getItem(key) || "null") || initial; setSettings({ ...saved, income: 0 }); setIncomeDraft(""); setIncomeUnlocked(false); setIncomePassphrase(""); } catch { setSettings(initial); } }, [key]);
  const saveSettings = (next: Settings) => { setSettings(next); localStorage.setItem(key, JSON.stringify({ ...next, income: 0 })); };
  useEffect(() => {
  ...
  const unlockIncome = async () => { setIncomeStatus("Unlocking…"); try { const result = await unlockPrivateIncome(date.slice(0, 7), incomePassphrase, token); setSettings((current) => ({ ...current, income: result.income })); setIncomeDraft(String(result.income)); setIncomeUnlocked(true); setIncomeStatus("Income unlocked for this session."); } catch (error) { setIncomeStatus(error instanceof Error ? error.message : "Could not unlock income."); } };
  const storeIncome = async () => { if (!Number(incomeDraft) || incomePassphrase.length < 8) { setIncomeStatus("Enter an income and a passphrase of at least 8 characters."); return; } setIncomeStatus("Encrypting and saving…"); try { const result = await savePrivateIncome(date.slice(0, 7), Number(incomeDraft), incomePassphrase, token); setSettings((current) => ({ ...current, income: result.income })); setIncomeUnlocked(true); setIncomeStatus("Encrypted income saved to Google Sheets."); } catch (error) { setIncomeStatus(error instanceof Error ? error.message : "Could not save income."); } };
  const lockIncome = () => { setSettings((current) => ({ ...current, income: 0 })); setIncomeDraft(""); setIncomePassphrase(""); setIncomeUnlocked(false); setIncomeStatus("Income locked."); };
  ...
  return <section className="manager-view">
    ...
    {section === "dashboard" && <>
      <article className="income-entry-card"><div><span className="eyebrow">Private monthly income</span><h3>{incomeUnlocked ? "Income unlocked" : "Income is hidden"}</h3><p>The passphrase is never stored as readable text.</p></div><div className="private-income-controls">{incomeUnlocked && <div className="income-entry-control"><span>₹</span><input inputMode="decimal" placeholder="Monthly income" value={incomeDraft} onChange={(e) => { if (/^\d*(\.\d{0,2})?$/.test(e.target.value)) setIncomeDraft(e.target.value); }} /></div>}<input type="password" placeholder="Income passphrase" value={incomePassphrase} onChange={(e) => setIncomePassphrase(e.target.value)} /><div>{!incomeUnlocked && <button onClick={unlockIncome}>Unlock</button>}<button onClick={storeIncome}>{incomeUnlocked ? "Update securely" : "Set new income"}</button>{incomeUnlocked && <button className="lock-income" onClick={lockIncome}>Lock</button>}</div>{incomeStatus && <small>{incomeStatus}</small>}</div></article>
      <div className="cashflow-grid"><article><span>Income</span><strong>{incomeUnlocked ? cash(settings.income) : "••••••"}</strong></article><article><span>Spent</span><strong>{cash(spent)}</strong></article><article className={incomeUnlocked ? remaining < 0 ? "danger" : "good" : ""}><span>Projected balance</span><strong>{incomeUnlocked ? cash(remaining) : "••••••"}</strong></article><article><span>Savings rate</span><strong>{incomeUnlocked && settings.income ? `${Math.round(remaining / settings.income * 100)}%` : "•••"}</strong></article></div>
    ...
    {section === "planning" && <>
      <article className="manager-card"><h3>Category budgets</h3><p className="muted">Private income is managed from the encrypted control on the Dashboard.</p><div className="budget-inputs">{expenseFields.map((f) => <label key={f.key}><span>{f.label}</span><input type="number" placeholder="No limit" value={settings.budgets[f.key] || ""} onChange={(e) => saveSettings({ ...settings, budgets: { ...settings.budgets, [f.key]: Number(e.target.value) } })} /></label>)}</div></article>
